...
export function generateReduceFractionSimpleDenominator(): GeneratorResult {
  ...
  let numerator = TermSum.generate(randInclusive(1, 3), () =>
...
export function generateAddFractionCommonDenominator(): GeneratorResult {
  ...
  let numerator1 = TermSum.generate(
    randInclusive(1, 2),
    Term.generateSimple,
    true,
  );
  let numerator2 = TermSum.generate(
    randInclusive(1, 2),
    Term.generateSimple,
    true,
  );
...
export function generateAddFraction(): GeneratorResult {
  ...
  const expansion1 = fraction1.denominator
    .lcm(fraction2.denominator)
    .divide(fraction1.denominator)!;
  const expansion2 = fraction1.denominator
    .lcm(fraction2.denominator)
    .divide(fraction2.denominator)!;
  ...
  fraction1 = new SimpleFraction(numerator1, denominator).simplifyNumerator(
    result.steps,
  );
  fraction2 = new SimpleFraction(numerator2, denominator).simplifyNumerator(
    result.steps,
  );
  ...
  let sum =
    sign(0) === '+'
      ? new SimpleFraction(
          fraction1.numerator.add(fraction2.numerator),
          denominator,
        ).simplifyNumerator(result.steps)
      : new SimpleFraction(
          fraction1.numerator.subtract(fraction2.numerator),
          denominator,
        ).simplifyNumerator(result.steps);
...
export function generateSeparateFraction(): GeneratorResult {
  ...
  let separatedFractions = fraction.numerator.terms.map((term) => {
    return new SimpleFraction(
      new TermSum([new Term(Math.abs(term.factor), term.variables)]),
      fraction.denominator,
      term.factor < 0 ? '-' : '+',
    );
  });
  if (separatedFractions[0].sign == '+') separatedFractions[0].sign = '';

  result.steps.push(separatedFractions.map((f) => f.render()).join(' '));
  result.solution = separatedFractions
    .map((f) => f.reduce().render())
    .join(' ');
  return result;
}
...
export function generateMultiplyFraction(): GeneratorResult {
  ...
  result.steps.push(
    ...simplifySteps.map((s) => `${s} * ${fraction2.render()}`),
  );
  simplifySteps = [];
  fraction2 = fraction2.simplifyNumerator(simplifySteps);
  result.steps.push(
    ...simplifySteps.map((s) => `${fraction1.render()} * ${s}`),
  );
  ...
  let product = new SimpleFraction(
    fraction1.numerator.multiply(fraction2.numerator),
    fraction1.denominator.multiply(fraction2.denominator),
  );

  result.steps.push(
    `<hidden>(${fraction1.numerator.renderAsFactor(true)} * ${fraction2.numerator.renderAsFactor()}) / (${fraction1.denominator.renderAsFactor(true)} * ${fraction2.denominator.renderAsFactor()})`,
  );
