...
import { generators, getSelectedCategories, setSelectedCategories, generatorConfig, onlyUseX, setOnlyUseX } from './generators/problemgenerators.js';
...
function filterDuplicateSteps(steps: string[], problem: string, solution: string) {
    ...
    for (let step of steps) {
        ...
        if (trimmedStep === problem || trimmedStep === solution)
            continue;
        if (prevStepTrimmed === trimmedStep)
            continue;
    ...
    if (steps) {
        ...
        for (let step of steps) {
            if (step.includes('<hidden>')) {
                const content = '<div class="p-2 md:p-4">`' + step.replace('<hidden>', '') + ' = `</div>';
                ...
                if (lastChild && lastChild.tagName === 'DETAILS')
                    ...
                    document.getElementById('steps')!.innerHTML += `<details class="hidden"><summary class="text-sm text-gray-500 cursor-pointer select-none">show intermediate step</summary>${content}</details>`;
            } else {
...
window.showNextStep = () => {
    ...
    if (step > document.getElementById('steps')!.childElementCount) {
        ...
        if (document.getElementById('steps')!.children[step - 1].tagName === 'DETAILS')
            window.showNextStep();
    }
}
...
function initSettings() {
    document.getElementById('settings')!.children[1].innerHTML = Object.keys(generatorConfig).map((c) =>
        ...
        `<label for="setting-cb-${c}">&nbsp;${generatorConfig[c as keyof typeof generatorConfig].name}</label></div>`).join('');

    if (onlyUseX()) {
        (<HTMLInputElement>document.getElementById('setting-var-x')!).checked = true;
    }
}
...
window.saveSettings = () => {
    const selectedCategories = Array.from(document.getElementById('settings')!.children[1].children)
        .filter((c) => (<HTMLInputElement>c.firstElementChild!).type === 'checkbox' && (<HTMLInputElement>c.firstElementChild!).checked).map((c) => (<HTMLInputElement>c.firstElementChild!).name);
    setSelectedCategories(selectedCategories.join(''));
    setOnlyUseX((<HTMLInputElement>document.getElementById('setting-var-x')!).checked);
